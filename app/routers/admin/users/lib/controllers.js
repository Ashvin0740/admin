...
controller.statistics = async (req, res) => {
  try {
    ...
    const gameQuery = [
      {
        $group: {
          ...
          nTotalBattle: {
            $sum: {
              $cond: [{ $eq: ["$aParticipant.iUserId", iUserId] }, 1, 0],
            },
          },
          nTotalWon: {
            $sum: { $cond: [{ $eq: ["$iWinnerId", iUserId] }, 1, 0] },
          },
        },
      },
    ];
    ...
    const [transactionResult, gameResult] = await Promise.all(
      Transaction.aggregate(transactionQuery),
      MetaGame.aggregate(gameQuery)
    );
    const response = {
      nTotalEarned: 0,
      nTotalClaim: 0,
